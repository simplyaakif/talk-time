        ...
        <div class="col-md-12">
            @can('talent_talk_time_create')
                <div style="margin-bottom: 10px;" class="row">
                    <div class="col-lg-12">
                        <a class="btn btn-success" href="{{ route('frontend.talent-talk-times.create') }}">
                                    ...
                                    <tr data-entry-id="{{ $talentTalkTime->id }}">
                                        ...
                                        <td>
                                            @can('talent_talk_time_show')
                                                <a class="btn btn-xs btn-primary" href="{{ route('frontend.talent-talk-times.show', $talentTalkTime->id) }}">
                                                    {{ trans('global.view') }}
                                                </a>
                                            @endcan

                                            @can('talent_talk_time_edit')
                                                <a class="btn btn-xs btn-info" href="{{ route('frontend.talent-talk-times.edit', $talentTalkTime->id) }}">
                                                    {{ trans('global.edit') }}
                                                </a>
                                            @endcan

                                            @can('talent_talk_time_delete')
                                                <form action="{{ route('frontend.talent-talk-times.destroy', $talentTalkTime->id) }}" method="POST" onsubmit="return confirm('{{ trans('global.areYouSure') }}');" style="display: inline-block;">
...
@can('talent_talk_time_delete')
  ...
  let deleteButton = {
    ...
    url: "{{ route('frontend.talent-talk-times.massDestroy') }}",
