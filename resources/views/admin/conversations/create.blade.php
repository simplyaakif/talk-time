...
<div class="card">
    <div class="card-header">
        {{ trans('global.create') }} {{ trans('cruds.conversation.title_singular') }}
    </div>

    <div class="card-body">
        <form method="POST" action="{{ route("admin.conversations.store") }}" enctype="multipart/form-data">
            @csrf
            ...
            <div class="form-group">
                ...
                <select class="form-control select2 {{ $errors->has('guide') ? 'is-invalid' : '' }}" name="guide_id" id="guide_id">
                    @foreach($guides as $id => $guide)
                        <option value="{{ $id }}" {{ old('guide_id') == $id ? 'selected' : '' }}>{{ $guide }}</option>
                    @endforeach
                </select>
                @if($errors->has('guide'))
                    <div class="invalid-feedback">
            ...
            <div class="form-group">
                ...
                <textarea class="form-control {{ $errors->has('guide_remarks') ? 'is-invalid' : '' }}" name="guide_remarks" id="guide_remarks">{{ old('guide_remarks') }}</textarea>
            ...
            <div class="form-group">
                ...
                <textarea class="form-control {{ $errors->has('talent_remarks') ? 'is-invalid' : '' }}" name="talent_remarks" id="talent_remarks">{{ old('talent_remarks') }}</textarea>
