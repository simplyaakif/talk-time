...
@can('talent_create')
    <div style="margin-bottom: 10px;" class="row">
        <div class="col-lg-12">
            <a class="btn btn-success" href="{{ route('admin.talent.create') }}">
                ...
                <tbody>
                    @foreach($talent as $key => $talent)
                        <tr data-entry-id="{{ $talent->id }}">
                            ...
                            <td>
                                @can('talent_show')
                                    <a class="btn btn-xs btn-primary" href="{{ route('admin.talent.show', $talent->id) }}">
                                        {{ trans('global.view') }}
                                    </a>
                                @endcan

                                @can('talent_edit')
                                    <a class="btn btn-xs btn-info" href="{{ route('admin.talent.edit', $talent->id) }}">
                                        {{ trans('global.edit') }}
                                    </a>
                                @endcan

                                @can('talent_delete')
                                    <form action="{{ route('admin.talent.destroy', $talent->id) }}" method="POST" onsubmit="return confirm('{{ trans('global.areYouSure') }}');" style="display: inline-block;">
...
@can('talent_delete')
  ...
  let deleteButton = {
    ...
    url: "{{ route('admin.talent.massDestroy') }}",
